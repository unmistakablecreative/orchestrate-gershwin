...
# === CONFIG AND DATABASE ===
CONFIG_PATH = os.path.join(os.path.dirname(__file__), '..', 'data', 'claude_assistant_config.json')
DB_PATH = os.path.join(os.path.dirname(__file__), '..', 'data', 'tasks.db')
STAGED_TASKS_FILE = os.path.join(os.path.dirname(__file__), '..', 'data', 'staged_tasks.json')
...
def get_db():
    ...
    cfg = load_config()
    db_rel = cfg.get('db_path', 'data/tasks.db')
    db_path = os.path.join(os.path.dirname(__file__), '..', db_rel)
...
def init_tasks_db():
    """Initialize tasks.db schema if not exists."""
    conn = get_db()
...
# === 7-QUEUE PARALLEL EXECUTION SYSTEM (LEGACY - kept for compatibility) ===
NUM_QUEUES = 7
...
def get_active_tasks(params):
    """Read-only view of queued/in_progress tasks from DB. Used by task board."""
    try:
        conn = get_db()
        rows = conn.execute(
            "SELECT * FROM tasks WHERE status IN ('queued','in_progress') ORDER BY created_at ASC"
        ).fetchall()
        conn.close()
        tasks = {}
        for row in rows:
            t = dict(row)
            try:
                t["context"] = json.loads(t.get("context") or "{}")
            except Exception:
                t["context"] = {}
            tasks[t["task_id"]] = t
...
def batch_stage_tasks(params):
    """Stage multiple tasks at once. Writes to STAGED_TASKS_FILE."""
    tasks = params.get("tasks")
    if not tasks:
        return {"status": "error", "message": "Missing required field: tasks"}
    if not isinstance(tasks, list) or len(tasks) == 0:
        return {"status": "error", "message": "tasks must be a non-empty list"}
    
    staged = []
    if os.path.exists(STAGED_TASKS_FILE):
        try:
            with open(STAGED_TASKS_FILE) as f:
                staged = json.load(f)
        except Exception:
            staged = []
    ...
    for i, task in enumerate(tasks):
        if not isinstance(task, dict) or not task.get("description"):
            errors.append({"index": i, "message": "Missing description"})
            continue
        staged.append({
            "description": task["description"],
            "preset": task.get("preset", "custom"),
            "staged_at": now
        })
        added += 1
    
    try:
        with open(STAGED_TASKS_FILE, 'w') as f:
            json.dump(staged, f, indent=2)
    except Exception as e:
        return {"status": "error", "message": f"Failed to write staged tasks: {str(e)}"}
    
    result = {
        "status": "success" if added > 0 else "error",
        "message": f"Staged {added}/{len(tasks)} tasks ({len(staged)} total)",
...
def log_task_completion_async(params):
    """Async wrapper for log_task_completion. Returns immediately."""
    return log_task_completion(params)
...
def self_assign_from_doc(params):
    ...
    """
    doc_id = params.get("doc_id", "51b72c69-0e28-4d20-bfbc-6b0854ec2f25")  # Claude Tasks in doc_editor
    ...
    try:
        ...
        result = subprocess.run(
            ["/Users/srinivas/venv/bin/python3", "execution_hub.py", "execute_task", "--params",
             json.dumps({"tool_name": "doc_editor", "action": "get_doc", "params": {"doc_id": doc_id}})],
            capture_output=True,
            text=True,
            timeout=30,
            cwd=os.getcwd(),
...
def main():
    ...
    # Action dispatcher (if/elif pattern per CLAUDE.md)
    if action == 'assign_task':
        result = assign_task(params)
    elif action == 'assign_demo_task':
        result = assign_demo_task(params)
    elif action == 'assign_mockup_batch':
        result = assign_mockup_batch(params)
    elif action == 'batch_assign_tasks':
    ...
    elif action == 'mark_task_in_progress':
        result = mark_task_in_progress(params)
    elif action == 'log_task_completion':
        result = log_task_completion(params)
    elif action == 'log_task_completion_async':
        result = log_task_completion_async(params)
    elif action == 'kill_agents':
        result = kill_agents(params)
    elif action == 'capture_token_telemetry':
        result = capture_token_telemetry(params)
    elif action == 'add_to_memory':
        result = add_to_memory(params)
    elif action == 'get_working_memory':
        result = get_working_memory(params)
    elif action == 'clear_working_memory':
        result = clear_working_memory(params)
    elif action == 'archive_thread_logs':
        result = archive_thread_logs(params)
    elif action == 'infer_task_type':
        result = infer_task_type(params)
    elif action == 'get_task_context':
        result = get_task_context(params)
    elif action == 'parse_tasks_from_doc':
        result = parse_tasks_from_doc(params)
    elif action == 'self_assign_from_doc':
        result = self_assign_from_doc(params)
    elif action == 'stage_task':
        result = stage_task(params)
    elif action == 'get_staged_tasks':
        result = get_staged_tasks(params)
    elif action == 'clear_staged_tasks':
        result = clear_staged_tasks(params)
    elif action == 'get_active_tasks':
        result = get_active_tasks(params)
    elif action == 'batch_stage_tasks':
        result = batch_stage_tasks(params)
    else:
        result = {
            'status': 'error',
