...
def convert_media(filename):
    ...
    return {'status': 'success', 'message':
        get_success_message('media_manager', 'upload', {'filename': filename})}
...
def download_youtube(url):
    ...
    with yt_dlp.YoutubeDL(opts) as ydl:
        ydl.download([url])
    return {'status': 'success', 'message':
        get_success_message('media_manager', 'upload', {'filename': url})}
...
def convert_file(filename, from_format, to_format):
    ...
    if from_format == 'txt' and to_format == 'md':
        ...
        return {'status': 'error', 'message':
            get_error_message('media_manager', 'upload', f'Conversion {from_format} → {to_format} not supported')}
    return {'status': 'success', 'message': get_success_message('media_manager', 'upload', {'filename': out_filename})}
...
def main():
    ...
    if args.action == 'compress_image':
        ...
        result = {'status': 'error', 'message': get_error_message('media_manager', 'upload', f'Unknown action {args.action}')}
