...
def create_doc(title: str, content: str, collection: str = "Notes", convert_markdown: bool = True) -> dict:
    ...
    # Append doc stamp from script_output_config.json
    stamp_config_path = os.path.join(BASE_DIR, "data", "script_output_config.json")
    if os.path.exists(stamp_config_path):
        try:
            with open(stamp_config_path, "r") as f:
                stamp_config = json.load(f)
            tool_stamp = stamp_config.get("tool_stamp_doc", "")
            if tool_stamp:
                html_content = html_content.rstrip() + f"\n\n<p><em>{tool_stamp}</em></p>"
        except Exception:
            pass  # Silently skip if config is unreadable
...
def sync_content_calendar() -> dict:
    ...
    Mirrors deprecated outline_editor.get_content_calendar but reads from SQLite docs.db.
    """
    conn = get_db()
    cursor = conn.cursor()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(json.dumps({"status": "error", "message": "No action provided"}))
        sys.exit(1)
    action = sys.argv[1]
    ...
    if action == "create_doc":
    ...
    elif action == "batch_create_docs":
        result = batch_create_docs(**params)
    elif action == "update_doc":
        result = update_doc(**params)
    elif action == "update_metadata":
        result = update_metadata(**params)
    elif action == "batch_update_metadata":
        result = batch_update_metadata(**params)
    elif action == "replace_section":
        result = replace_section(**params)
    elif action == "append_doc":
        result = append_doc(**params)
    elif action == "delete_doc":
        result = delete_doc(**params)
    elif action == "delete_collection":
        result = delete_collection(**params)
    elif action == "read_doc":
        result = read_doc(**params)
    elif action == "read_section":
        result = read_section(**params)
    elif action == "batch_read":
        result = batch_read(**params)
    elif action == "search_within_doc":
        result = search_within_doc(**params)
    elif action == "list_docs":
        result = list_docs(**params)
    elif action == "search_docs":
        result = search_docs(**params)
    elif action == "batch_search":
        result = batch_search(**params)
    elif action == "import_docs":
        result = import_docs(**params)
    elif action == "migrate_from_outline":
        result = migrate_from_outline(**params)
    elif action == "sync_content_calendar":
        result = sync_content_calendar()
    elif action == "link_docs":
        result = link_docs(**params)
    elif action == "unlink_docs":
        result = unlink_docs(**params)
    elif action == "read_backlinks":
        result = read_backlinks(**params)
    elif action == "read_links":
        result = read_links(**params)
    elif action == "retrieve_doc":
        ...
        result = get_revision(**params)
    elif action == "restore_revision":
        result = restore_revision(**params)
    else:
