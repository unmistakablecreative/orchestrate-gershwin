...
def init_logs_db():
    """Initialize logs.db with execution_logs table if it doesn't exist."""
...
def log_execution(tool, action, params, status, result, duration_ms=None):
    ...
    try:
        # Rotate debug logs periodically (every 10th execution)
        state = read_json(THREAD_STATE_FILE, default={})
        ...
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO execution_logs (timestamp, tool_name, action, status, duration_ms, error)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (timestamp, tool, action, status, duration_ms, error_msg))
        conn.commit()
        conn.close()
...
def main():
    # Initialize SQLite logs database on startup
    init_logs_db()
